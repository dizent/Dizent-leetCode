package nowcoder.maxIncreaseKeepingSkyline;
...
public class Solution {
    ...
    public int maxIncreaseKeepingSkyline(int[][] grid) {
        ...
        for(int i = 0;i < grid.length;i++){
            for(int j = 0;j< grid[i].length;j++){
                addCount += Math.min(horizon[i],vertical[j]) - grid[i][j];
                grid[i][j] = Math.min(horizon[i],vertical[j]);
            }
        }
        print2ThArr(grid);
    ...
    public void print2ThArr(int[][] board) {
        System.out.println("┏━━━┳━━━┳━━━┳━━━┓");
